...
do_install:append:aos-main-node() {
    ...
    echo "/dev/aosvg/storages  /var/aos/storages    ext4 \
        defaults,nofail,noatime,usrjquota=aquota.user,jqfmt=vfsv0,x-systemd.automount  0  0" >> ${D}${sysconfdir}/fstab 
...
do_install:append:aos-secondary-node() {
    # add Aos partitions
    echo "${AOS_MAIN_NODE_HOSTNAME}:/var/aos/storages /var/aos/storages    nfs4 \
        defaults,nofail,noatime,x-systemd.automount    0  0" >> ${D}${sysconfdir}/fstab
    echo "${AOS_MAIN_NODE_HOSTNAME}:/var/aos/states   /var/aos/states      nfs4 \
        defaults,nofail,noatime,x-systemd.automount    0  0" >> ${D}${sysconfdir}/fstab
}
