...
def call_bitbake(yocto_dir, conf):
    ...
    cmd = [
        f"cd {yocto_dir}",
        f'. ./poky/oe-init-build-env {conf["dir"]}',
...
def main():
    ...
    for key in layers["items"]:
        if enabled_layers:
            enabled = key in enabled_layers
        else:
            enabled = layers["items"][key].get("enabled", True)
        if enabled:
            ret = call_bitbake(yocto_dir, layers["items"][key])
    return ret
