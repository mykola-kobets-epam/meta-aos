...
from wic.misc import (BOOTDD_EXTRA_SPACE, exec_cmd, exec_native_cmd,
                      get_bitbake_var)
...
class BootimgEFIPlugin(SourcePlugin):
    ...
    def do_prepare_partition(
        ...
        if source_params.get("create-unified-kernel-image") == "true":
            ...
            if not initrd:
                raise WicError(
                    "initrd= must be specified when create-unified-kernel-image=true, exiting"
                )
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                objcopy_cmd += (
                    " --add-section .osrel=%s/usr/lib/os-release" % staging_dir_host
                )
        ...
        if blocks < part.fixed_size:
            ...
            logger.debug(
                "Overriding %s to %d total blocks for compatibility",
                part.mountpoint,
                blocks,
            )
